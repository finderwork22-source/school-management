...
export default function Students() {
  ...
  const visibleClassesForTeacher = useMemo(() => {
    if (!isTeacher) return classesForSelectedYear;

    return classesForSelectedYear.filter((item) =>
      teacherClassIds.includes(item.id),
    );
  }, [classesForSelectedYear, isTeacher, teacherClassIds]);
  ...
  const sectionsForSelectedYear = useMemo(() => {
    ...
    const visibleSectionIds = isTeacher
      ? new Set(visibleClassesForTeacher.map((item) => item.academic_section_id))
      : null;
    ...
    academicSections
      .filter(
        (section) =>
          ...
          (!visibleSectionIds || visibleSectionIds.has(section.id)),
      )
  ...
  const classesForSelectedSection = useMemo(() => {
    if (sectionFilter === "All sections") return visibleClassesForTeacher;
    const section = sectionsForSelectedYear.find(
      (item) => item.name === sectionFilter,
    );
    return visibleClassesForTeacher.filter(
      (item) => item.academic_section_id === section?.id,
    );
  }, [visibleClassesForTeacher, sectionFilter, sectionsForSelectedYear]);
  ...
  return (
    <div className="mx-auto w-full min-w-0 max-w-[1400px]">
      <PageHeader
        ...
        description={
          isTeacher
            ? "View students assigned to the classes you teach."
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
            ...
            <div className="grid min-w-0 grid-cols-1 gap-2 sm:flex sm:flex-wrap">
              <select
                ...
                className="h-10 w-full sm:w-auto rounded-lg border border-slate-200 bg-white px-3 text-sm text-slate-700 outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100"
              >
                {sections.map((item) => (
              ...
              <Button className="w-full sm:w-auto" variant="secondary" size="md">
        ...
        {isTeacher && !error && (
          <div className="border-b border-indigo-100 bg-indigo-50 px-4 py-3 sm:px-5">
            <p className="text-sm text-indigo-700">
              You are viewing only students assigned to your teaching classes.
            </p>
          </div>
        )}
        ...
        {loading ? (
          ...
          <>
                      ...
                      <td className="px-5 py-4">
                        ...
                        >
                          ...
                          <div className="min-w-0">
                            <div className="break-words text-sm font-medium text-slate-900 group-hover:text-indigo-700">
                              {student.name}
                            </div>
            ...
            <div className="divide-y divide-slate-100 md:hidden">
              {filteredStudents.map((student) => (
                <div key={student.id} className="min-w-0 p-4">
...
function AddStudentModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-900/40 p-0 sm:items-center sm:p-4">
      <div className="flex max-h-[100vh] w-full flex-col overflow-hidden rounded-t-2xl bg-white shadow-xl sm:max-h-[90vh] sm:max-w-xl sm:rounded-2xl">
        {/* Header */}
        <div className="flex min-w-0 items-center justify-between gap-3 border-b border-slate-200 px-4 py-4 sm:px-6">
          <div>
      ...
      <div className="flex max-h-[100vh] w-full flex-col overflow-hidden rounded-none bg-white shadow-xl sm:max-h-[90vh] sm:max-w-lg sm:rounded-2xl">
        <div className="flex min-w-0 items-start justify-between gap-3 border-b border-slate-200 p-4 sm:p-6">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              <h2 className="break-words font-semibold text-slate-900">{student.name}</h2>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto grid grid-cols-1 gap-px bg-slate-200 sm:grid-cols-2">
